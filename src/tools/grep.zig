...
pub fn execute(allocator: std.mem.Allocator, io: std.Io, args_json: []const u8) !ToolResult {
    ...
    if (query.len == 0) {
        return ToolResult{
            .success = false,
            .output = try allocator.dupe(u8, ""),
            .error_message = try allocator.dupe(u8, "Query string cannot be empty"),
        };
    }
    ...
    while (try walker.next(io)) |entry| {
        ...
        while (lines.next()) |raw_line| {
            ...
            if (case_sensitive) {
                found = std.mem.indexOf(u8, line, query) != null;
            } else {
                found = std.ascii.indexOfIgnoreCase(line, query) != null;
            }
    ...
    if (match_count == 0) {
        try out_list.appendSlice(allocator, "No matches found for query.");
    }
